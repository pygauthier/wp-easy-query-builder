	...
	public function query($query, $args = [], $quick = false){
		...
		if ($quick == true) {
			...
			if (strpos( strtoupper($query), "SELECT" ) === 0 ) {
				...
				$stmt->execute($this->bindValues);
				$this->rowCount = $stmt->rowCount();
				$rows = $stmt->fetchAll(PDO::FETCH_CLASS,'ewpdbObj');
				$collection= [];
				$collection = new ewpdbCollection;
				$x=0;
...
class ewpdbCollection implements ArrayAccess{
       ...
       public function toArray(){
        ...
        $array = [];
        foreach ($this as  $ewpdbObj) {
          $array[] = (array) $ewpdbObj;
        }
           return $array;
       }
